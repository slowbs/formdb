...
try {
    ...
    if ($stmt->rowCount() < 1){
        ...
        foreach($stmt2->fetchAll() as $k=>$v) {
            ...
            $condition2 = "INSERT INTO ".$tbname." (". substr($field_name, 0, -2) . ", `date_created`) SELECT * FROM `".$ran."` WHERE NOT EXISTS ( SELECT * FROM `".$tbname."` where "
            ...
            $stmt->execute();
            echo "เพิ่มทั้งหมด " . $count . " รายการ";
            /* echo $cretable ."<BR>";
            echo $string4 ."<BR>";
            echo $condition2; */
